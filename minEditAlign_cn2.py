...
def wagner_fischer(word_1, word_2, timing_1, timing_2, timalign, dele, ins, sub):
    ...
    for i, l_1 in enumerate(word_1, start=1):
	    ...
	    #print l_1, l_2, timing_1[i-1],timing_2[j-1],D[i-1,j],D[i, j-1], D[i-1,j-1],idt, ct,i,j
            ...
            deletion = D[i-1,j] + dele
            insertion = D[i, j-1] + ins
            substitution = D[i-1,j-1] + (0 if l_1==l_2 else sub)
...
def extractMLF(alpha, noneSc, file1, file2, timalign, decode1, decode2, dele, ins, sub):
	...
	for i in range(1, len(recs1)):
			...
			D, B = wagner_fischer(words_1, words_2, timing_1, timing_2, timalign,  dele, ins, sub)
			bt = naive_backtrace(B)	 
...
def main() :
    ...
    parser.add_argument('--d2', dest='decode2', action='store', required=True,
                        help='file 2')
    parser.add_argument('--del', dest='dele', action='store', default=2,
                        help='file 2')
    parser.add_argument('--ins', dest='ins', action='store', default=2,
                        help='file 2')
    parser.add_argument('--sub', dest='sub', action='store', default=2,
                        help='file 2')
   
    args = parser.parse_args()
    extractMLF(float(args.alpha), float(args.noneSc), args.file1, args.file2, float(args.timalign), args.decode1, args.decode2, float(args.dele), float(args.ins), float(args.sub))
